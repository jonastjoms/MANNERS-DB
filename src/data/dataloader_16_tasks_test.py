...
def get(data_path):
    ...
    for i, action in enumerate(actions[0]):
        ...
        data[i]['name'] = action
        data[i]['n_outputs'] = 2
        data[i]['test']= {'x': torch.tensor([df_circle[i].iloc[:,0:-8].values])[0],'y': torch.tensor([df_circle[i].iloc[:,-8+i].values])[0]}
    ...
    for i, action in enumerate(actions[1]):
        ...
        data[i+8]['test']= {'x': torch.tensor([df_arrow[i].iloc[:,0:-8].values])[0],'y': torch.tensor([df_arrow[i].iloc[:,-8+i].values])[0]}
